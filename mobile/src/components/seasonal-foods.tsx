...
const seasons = [
  { key: '봄', months: '3–5월', title: '향긋하게 입맛을 깨우는 계절', color: '#fff1f5', border: '#f5c8d5', accent: '#d85c7f', foods: [
    ['도다리쑥국', '담백한 도다리와 향긋한 쑥'], ['주꾸미볶음', '탱글한 식감과 매콤한 양념'], ['봄나물 비빔밥', '냉이·달래·참나물을 한 그릇에'], ['바지락 칼국수', '제철 바지락으로 낸 시원한 국물'],
  ] },
  { key: '여름', months: '6–8월', title: '시원하게 채우는 여름 한 끼', color: '#eaf8ff', border: '#b9dfef', accent: '#167f9f', foods: [
    ['평양냉면', '슴슴하고 차가운 육수의 매력'], ['초계국수', '새콤한 육수와 담백한 닭고기'], ['삼계탕', '기운을 채워주는 대표 보양식'], ['물회', '제철 회와 채소를 시원하게'],
  ] },
  { key: '가을', months: '9–11월', title: '불향과 감칠맛이 깊어지는 계절', color: '#fff3e3', border: '#edcfaa', accent: '#a65f29', foods: [
    ['전어구이', '고소한 기름과 진한 불향'], ['대하구이', '소금 위에서 구워낸 탱글한 식감'], ['꽃게탕', '달큰한 꽃게와 칼칼한 국물'], ['송이버섯 전골', '가을 버섯의 깊고 은은한 향'],
  ] },
  { key: '겨울', months: '12–2월', title: '따뜻하고 든든하게 즐기는 계절', color: '#eef3ff', border: '#c7d4ef', accent: '#4f6fae', foods: [
        ...
        {seasons.map((item) => (
          <MotionPressable
            ...
            style={[
              ...
              selected === item.key && { backgroundColor: item.accent, borderColor: item.accent, shadowColor: item.accent },
            ]}
          >
            <Text maxFontSizeMultiplier={1.15} style={[styles.tabText, selected === item.key && styles.tabTextSelected]}>{item.key}</Text>
          </MotionPressable>
        ))}
      </View>
      <View style={[styles.card, { backgroundColor: season.color, borderColor: season.border }]}>
        <View style={[styles.cardAccent, { backgroundColor: season.accent }]} />
        <Text maxFontSizeMultiplier={1.15} style={[styles.months, { color: season.accent }]}>{season.key} · {season.months}</Text>
...
const styles = StyleSheet.create({
  ...
  tabs: { marginTop: 16, flexDirection: 'row', gap: 8 }, tab: { flex: 1, minHeight: 46, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderRadius: 14, shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.18, shadowRadius: 8, elevation: 2 },
  tabText: { color: '#4f4f4b', fontSize: 13, lineHeight: 18, fontWeight: '900', textAlign: 'center' }, tabTextSelected: { color: '#ffffff' },
  card: { marginTop: 12, overflow: 'hidden', borderWidth: 1, borderRadius: 22, padding: 18 }, cardAccent: { height: 5, marginHorizontal: -18, marginTop: -18, marginBottom: 17 }, months: { fontSize: 11, fontWeight: '900', letterSpacing: 0.6 }, cardTitle: { marginTop: 5, marginBottom: 8, color: '#101010', fontSize: 19, fontWeight: '900' },
