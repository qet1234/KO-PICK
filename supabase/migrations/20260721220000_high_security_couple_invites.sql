...
-- Returns the claimed couple id, or null for an invalid/unavailable code.
...
begin
  ...
  -- Serialize attempts from the same account to close concurrency gaps.
  ...
  if p_invite_code is null
     or length(trim(p_invite_code)) < 12
     or length(trim(p_invite_code)) > 128 then
    return null;
  end if;

  -- A user may belong to only one couple.
  if exists (
  ...
  if not exists (
    ...
    raise exception 'Could not create membership' using errcode = 'P0001';
  end if;
...
-- Membership helper using the real schema.
...
-- Shared data tables are couple-member only.
do $$
declare
  v_table text;
begin
...
-- Prevent accidental direct exposure of invite secrets to browser roles.
...
-- Remove stale rate-limit rows without exposing a public maintenance function.
delete from public.couple_invite_attempts
