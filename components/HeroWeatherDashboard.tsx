...
type Subregion = { code: string; name: string };
    ...
    async function loadSubregions() {
      try {
        const response = await fetch(
          `${springApiUrl}/api/public/tour/places?mode=subregions&region=${encodeURIComponent(region)}`,
          { cache: "no-store" },
        );
        const payload = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(payload.error ?? "세부 지역 조회 실패");
        if (!cancelled) setSubregions(Array.isArray(payload.subregions) ? payload.subregions : []);
      } catch {
    ...
    <article className="kp-weather-dashboard" aria-label="전국 실시간 날씨">
      <header className="kp-weather-dashboard-header">
        <div>
          ...
          <h1>전국 실시간 날씨</h1>
          <p>시·도와 시·군·구를 선택해 현재 날씨와 시간대별 예보를 확인하세요.</p>
        </div>
        <span className="kp-weather-live"><i />10분 자동 갱신</span>
      </header>

      <div className="kp-weather-location-controls">
        <label>
          <span>시·도</span>
        ...
        <label>
          ...
          <select value={district} onChange={(event) => setDistrict(event.target.value)}>
            ...
            {subregions.map((item) => <option key={item.code || item.name} value={item.name}>{item.name}</option>)}
          </select>
        </label>
      </div>

      {loading ? (
        <div className="kp-weather-dashboard-state">선택 지역의 실시간 날씨를 불러오고 있습니다.</div>
      ) : error || !weather ? (
            ...
            <div className="kp-weather-weekly-list">
              {weather.daily.map((item, index) => (
                <article key={item.date}>
                  <small>{formatDay(item.date, index)}</small>
                  <span aria-hidden="true">{item.icon}</span>
